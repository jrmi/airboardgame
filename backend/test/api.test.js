...
const user = crypto
  ...
  .digest("hex");
const otherUser = crypto
  .createHash("sha256")
  .update("other@example.com")
  .digest("hex");
const cookie = (id) => {
  const signature = crypto
    .createHmac("sha256", "api-test-secret")
    .update(id)
    .digest("base64url");
...
const call = async (route, options = {}) => {
  ...
  if (requestBody instanceof FormData) {
    const prepared = new Request("http://test", {
  ...
  return {
    response: {
      ...
      headers: { get: (name) => response.getHeader(name) },
    },
    body: parsedBody,
  };
};

const json = (method, body, userId) => ({
  method,
  user: userId,
...
describe("HTTP API", () => {
  ...
  test("login request and verification consume the token", async () => {
      ...
      assert.equal(
        (
          await call("/auth/", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ userEmail: "Owner@Example.com" }),
          })
        ).response.status,
        200
      );
    ...
    assert.match(
      ...
      new RegExp(`session=${userId}\\.`)
    );
    assert.equal(
      (await call(`/auth/verify/${userId}/${token}`)).response.status,
      403
    );
  });
  ...
  test("game list filters unpublished games and supports query fields", async () => {
    ...
    await call(
      "/store/game/public",
      json("POST", { board: { published: true }, title: "Public" }, otherUser)
    );
    ...
    assert.deepEqual(owner.body.map(({ _id }) => _id).sort(), [
      "private",
      "public",
    ]);
  });
  ...
  test("game CRUD enforces ownership and preserves the owner", async () => {
    const created = await call(
      "/store/game/g1",
      json("POST", { board: { published: false }, owner: otherUser }, user)
    );
    ...
    assert.equal(
      (
        await call(
          "/store/game/g1",
          json("POST", { title: "no", owner: otherUser }, otherUser)
        )
      ).response.status,
      403
    );
    assert.equal(
      (await call("/store/game/g1", { user })).body.title,
      undefined
    );
    assert.equal(
      (await call("/store/game/g1", { method: "DELETE", user: otherUser }))
        .response.status,
      403
    );
    assert.deepEqual(
      (await call("/store/game/g1", { method: "DELETE", user })).body,
      { message: "Deleted" }
    );
    assert.equal((await call("/store/game/g1")).response.status, 404);
    assert.equal(
      (await call("/store/game/g2", json("POST", {}))).response.status,
      403
    );
  });
  ...
  test("session, room, and user APIs support CRUD and account protection", async () => {
    assert.equal(
      (await call("/store/session/s1", json("POST", { timestamp: Date.now() })))
        .response.status,
      200
    );
    assert.deepEqual((await call("/store/session/s1")).body._id, "s1");
    assert.equal(
      (await call("/store/room/r1", json("POST", { members: [] }))).response
        .status,
      200
    );
    assert.deepEqual((await call("/store/room/r1")).body.members, []);
    assert.equal(
      (await call(`/store/user/${user}`, { user })).response.status,
      200
    );
    assert.equal((await call(`/store/user/${user}`, { user })).body._id, user);
    assert.equal(
      (await call(`/store/user/${otherUser}`, { user })).response.status,
      403
    );
  ...
  test("media upload, list, download, and deletion use game authorization", async () => {
    ...
    const uploaded = await call("/store/game/media/file/", {
      method: "POST",
      body: form,
      user,
    });
    ...
    assert.match(
      uploaded.body,
      new RegExp(`^${site}/store/game/media/file/[a-f0-9]{32}\\.txt$`)
    );
    ...
    assert.equal(
      (await call(`/store/game/media/file/missing-${filename}`)).response
        .status,
      404
    );
    assert.equal(
      (
        await call(`/store/game/media/file/${filename}`, {
          method: "DELETE",
          user: otherUser,
        })
      ).response.status,
    ...
    assert.equal(
      (await call("/store/game/no-file/file/", { method: "POST", user }))
        .response.status,
      400
    );
  });
...
describe("services and recurring cleanup", () => {
  ...
  test("media path and filename helpers prevent path traversal", () => {
    ...
    assert.equal(
      mediaPath("g", "image.png"),
      `${site}/store/game/g/file/image.png`
    );
  });
  ...
  test("admin can modify another user's game", async () => {
    ...
    await gameService.save(
      "game",
      "admin-game",
      { title: "initial", owner: otherUser },
      otherUser
    );
    const updated = await gameService.save(
      "game",
      "admin-game",
      { title: "updated" },
      user
  ...
  test("creates a session and requests a connection token", async () => {
    ...
    globalThis.fetch = async (url, options) => {
      ...
      return calls.length === 1
        ? new Response(JSON.stringify({ id: "actual" }), { status: 200 })
    ...
    try {
      ...
      assert.equal(
        ...
        `Basic ${Buffer.from("OPENVIDUAPP:secret").toString("base64")}`
      );
      assert.match(calls[1].url, /sessions\/actual\/connection$/);
    } finally {
      globalThis.fetch = previous.fetch;
      if (previous.url === undefined) delete process.env.OPENVIDU_URL;
      else process.env.OPENVIDU_URL = previous.url;
      if (previous.secret === undefined) delete process.env.OPENVIDU_SECRET;
      else process.env.OPENVIDU_SECRET = previous.secret;
    }
  });
});
