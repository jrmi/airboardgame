...
const io = new Server(server, { path: process.env.SOCKET_PATH || "/socket.io", cors: { origin: true, credentials: true } });
configureRealtime(io);
const host = process.env.SERVER_HOST || "localhost";
const port = Number(process.env.SERVER_PORT || 4050);
server.listen(port, host, () => console.log(`Airboardgame backend listening on http://${host}:${port}`));
