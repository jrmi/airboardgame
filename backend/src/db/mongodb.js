...
const cursor = (store, query) => {
  const options = {};
  return {
    sort(value) { options.sort = value; return this; },
    skip(value) { options.skip = value; return this; },
    limit(value) { options.limit = value; return this; },
    toArray() {
      return new Promise((resolve, reject) => {
        ...
        result.exec((error, documents) => error ? reject(error) : resolve(documents));
      });
    },
  };
};
...
const nedbCollection = async (box) => {
  ...
  return {
    ...
    findOne: (query) => new Promise((resolve, reject) => store.findOne(query, (error, result) => error ? reject(error) : resolve(result))),
    insertOne: (document) => new Promise((resolve, reject) => store.insert(document, (error, result) => error ? reject(error) : resolve({ insertedId: result._id }))),
    replaceOne: (query, document, { upsert = false } = {}) => new Promise((resolve, reject) => store.update(query, document, { multi: false, upsert }, (error, count) => error ? reject(error) : resolve({ modifiedCount: count, deletedCount: count }))),
    deleteOne: (query) => new Promise((resolve, reject) => store.remove(query, { multi: false }, (error, count) => error ? reject(error) : resolve({ deletedCount: count }))),
    deleteMany: (query) => new Promise((resolve, reject) => store.remove(query, { multi: true }, (error, count) => error ? reject(error) : resolve({ deletedCount: count }))),
  };
};

export const getCollection = async (box) =>
  useNeDb() ? nedbCollection(box) : (await connectMongo()).collection(collectionName(box));
